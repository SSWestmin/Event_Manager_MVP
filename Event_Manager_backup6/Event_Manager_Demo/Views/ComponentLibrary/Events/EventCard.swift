...
struct EventCard: View {
    ...
    //    Data access: coordinator.selectedEvent(for: eventID) refactored this function in the coordinator. The ViewModel exposes it to the view
    
    // Refactor: delegate event lookup through the view model so the card does not reach into the coordinator directly.
    var event: EventModel? {
        viewModel.selectedEvent(for: eventID)
    }
            ...
            ZStack {
                ...
                VStack(spacing: 20) {
                    ...
                    if userRole == "admin" {
                        ...
                        Text("Manage events")
                            .font(.title)
                        // FIXING HERE: CreateEventView now requires the shared viewModel so the admin create flow uses the same state.
                    ...
                    if userRole == "admin" {
                        ...
                        Button {
                            if let event = event {
                                // MARK: Check SwiftData for duplicate via coordinator
                                // FIXING HERE: use the card's localSavedEvents state instead of an undeclared coordinator reference.
