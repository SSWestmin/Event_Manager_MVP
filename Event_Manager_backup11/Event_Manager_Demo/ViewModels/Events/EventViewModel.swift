...
class EventViewModel: ObservableObject {
    ...
    func updateEvent(_ updated: EventModel) {
        coordinator.updateEvent(updated)
    }

    func deleteEvent(id: UUID) {
        coordinator.deleteEvent(id: id)
    }
    ...
    //    MARK: Refactor - after removing filtering from mapkit VM add here
    
    var filteredMapEvents: [EventModel] {
        filter(events: coordinator.events.filter {
    ...
    // MARK: FIXME - CORE FILTER LOGIC ACROSS MAPS, CALENDARS, LIST
